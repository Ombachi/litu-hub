-- Audit-style detail RPC for a parent-student link request.
-- Returns the link record (with requester/approver names) + related notifications
-- (parent-link request, approval, rejection messages) so admins can see the full
-- timeline of who requested, who approved/rejected, when, and what notifications fired.
...
BEGIN
  ...
  -- Authorization: platform admin OR scoped school admin
  ...
  -- Build the link payload with parent / student / approver names (no emails leaked beyond what admins already see).
  ...
  -- Related notifications: those sent to the parent about this linking flow,
  -- plus the admin-fanout "New Parent Link Request" notifications around the
  -- request time. Cap to recent + relevant ones.
  SELECT COALESCE(jsonb_agg(n ORDER BY n.created_at), '[]'::jsonb) INTO v_notifications
  FROM (
    SELECT jsonb_build_object(
      'id', n.id,
      'user_id', n.user_id,
    ...
    WHERE
      -- Parent-side notifications about this link
      ...
      OR
      -- Admin fan-out request notifications around request time
...
REVOKE ALL ON FUNCTION public.get_parent_link_audit(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.get_parent_link_audit(uuid) TO authenticated;