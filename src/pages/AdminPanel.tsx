...
import {
  Users, Shield, BookOpen, ClipboardList, Plus, Edit, Trash2, Loader2, Search, X, Save,
} from "lucide-react";
...
const AdminPanel = () => {
  ...
  const [courseForm, setCourseForm] = useState({ open: false, title: "", code: "", description: "", editing: null as any });
  ...
  const createCourse = useMutation({
    ...
    onSuccess: () => {
      ...
      setCourseForm({ open: false, title: "", code: "", description: "", editing: null });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">Manage users, roles, courses, and view audit logs</p>
      </div>
      ...
      <Tabs defaultValue="users" className="w-full">
                    ...
                    return (
                      <tr key={p.id} className="border-b last:border-0 hover:bg-secondary/20 transition-colors">
                        ...
                        <td className="px-5 py-3">
                          {isEditing ? (
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => updateRole.mutate({ userId: p.user_id, role: editingRole.role })}
                                disabled={updateRole.isPending}
                                className="p-1 rounded hover:bg-success/10 text-success"
                              >
                                <Save className="h-3.5 w-3.5" />
                              </button>
                              <button onClick={() => setEditingRole(null)} className="p-1 rounded hover:bg-secondary">
                        ...
                        <td className="px-5 py-3 text-right">
                          {!isEditing && (
                            <button
                              onClick={() => setEditingRole({ userId: p.user_id, role })}
        ...
        <TabsContent value="courses" className="mt-6 space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={() => setCourseForm({ open: true, title: "", code: "", description: "", editing: null })}
          ...
          {courseForm.open && (
            <div className="rounded-xl border bg-card p-5 shadow-card space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setCourseForm({ ...courseForm, open: false })} className="p-1 hover:bg-secondary rounded">
                  <X className="h-4 w-4" />
                </button>
              </div>
          ...
          {!courses?.length ? (
            ...
            courses.map((c) => (
              <div key={c.id} className="rounded-xl border bg-card p-5 shadow-card flex items-center justify-between">
                ...
                <button
                  onClick={() => { if (confirm(`Delete "${c.code}"?`)) deleteCourse.mutate(c.id); }}
